...
const x1 = mx(BBOX.e)
const y0 = my(BBOX.n)
const y1 = my(BBOX.s) // y растёт вниз в экранных координатах
const scale = WIDTH / (x1 - x0)
const HEIGHT = Number(Math.abs((y1 - y0) * scale).toFixed(1))
...
function simplify(pts, tol) {
  ...
  while (stack.length > 0) {
    ...
    for (let i = a + 1; i < b; i++) {
      const [px, py] = pts[i]
      const raw = len ? ((px - ax) * dx + (py - ay) * dy) / len : 0
...
/** Допуск упрощения по слоям: мелкие улицы можно грубить сильнее магистралей. */
const TOLERANCE = { minor: 1.4, green: 1.6, water: 1.6 }

const LINK_SUFFIX = /_link$/
const HW = (t) => (t || '').replace(LINK_SUFFIX, '')
...
for (const el of osm.elements) {
  ...
  if (t.leisure === 'park' || t.leisure === 'garden') {
    layer = 'green'
  } else if (['forest', 'grass', 'meadow', 'cemetery'].includes(t.landuse)) {
    layer = 'green'
  ...
  if ((layer === 'green' || layer === 'water') && span < 9) {
    continue
  }
  if (layer === 'minor' && span < 4) {
    continue
  }

  const flat = []
  for (const [x, y] of pts) {
    flat.push(Number(x.toFixed(1)), Number(y.toFixed(1)))
  }
...
const labels = places.elements
  ...
  .map((e) => {
    ...
    return { n: e.tags.name, x: Number(x.toFixed(1)), y: Number(y.toFixed(1)), k: e.tags.place }
  })
